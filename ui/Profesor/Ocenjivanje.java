	...
	Ocenjivanje(){
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				boolean val = ou.ValidacijUnosa(id);
				if(val == true) {
					boolean proveraID = false;
					try {
						 proveraID = ou.ProveraPostojanjaID(id);
		...
		btnSacuvaj.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(val2) {	
					...
					String nizIdPr[] = null;
					try {
						nizIdPr = ou.TrazenjeJmbgIPredmet(niz);
					} catch (IOException e1) {
						e1.printStackTrace();
					}
					String id = textField.getText();
					String idocene = null;
					try {
						idocene = String.valueOf(ou.IdOcene()+1);
					} catch (IOException e2) {
						e2.printStackTrace();
					}
					try {
						ou.UpisivanjeOcene(nizIdPr, id, ocena, opis, idocene);
					} catch (IOException e1) {
						e1.printStackTrace();
					}
					textField.setText("");
