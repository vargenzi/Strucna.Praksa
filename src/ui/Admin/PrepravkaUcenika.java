...
public class PrepravkaUcenika extends JFrame{
	...
	private String godine[] = {
			"-", "2002", "2003", "2004", "2005", "2005", "2006", "2007", "2008", "2009", "2010", "2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018", "2019", "2020", "2021"
	};
	...
	PrepravkaUcenika(){
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					provera = pu.ProveraPostojanjaID(tfid);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(textField_1.isEnabled()) {
					...
					if(textField_1.getText().equals("") == false) {
						try {
							pu.PostaviIme(textField_1.getText(), pomocni);
				...
				if(textField_2.isEnabled()) {
					...
					if(textField_2.getText().equals("") == false) {
						try {
							pu.PostaviPrezime(textField_2.getText(), pomocni);
						} catch (ClassNotFoundException | SQLException e1) {
							e1.printStackTrace();
						}
				...
				if(comboBox.isEnabled()) {
					...
					String datum = "";
					boolean poklapanje_godina = false;
					try {
						datum = baza.FormiranjeDatuma(comboBox.getSelectedItem().toString(), comboBox_1_1.getSelectedItem().toString(), comboBox_2.getSelectedItem().toString());
						String cont[] = pu.VratiGodinuRazred(pomocni);
						poklapanje_godina = pu.PoklapanjeGodina(pomocni, datum, cont);	
					} catch (ClassNotFoundException | SQLException e2) {
						e2.printStackTrace();
					};
					
					if(comboBox.getSelectedItem().toString().equals("-") || comboBox_1_1.getSelectedItem().toString().equals("-") || comboBox_2.getSelectedItem().toString().equals("-") || poklapanje_godina) {
						pu.Greska("Pogresan unos.");
					}
					
					if((comboBox.getSelectedItem().toString().equals("-") || comboBox_1_1.getSelectedItem().toString().equals("-") || comboBox_2.getSelectedItem().toString().equals("-") || poklapanje_godina) == false) {
						try {
							pu.PostaviDanMesecGodinu(datum, pomocni);
						} catch (ClassNotFoundException | SQLException e1) {
							e1.printStackTrace();
						}
				...
				if(comboBox_1_1_1.isEnabled()) {
					boolean poklapanje_godina = false;
					try {
						String datum = pu.VratiDatumRodjenja(pomocni);
						String cont[] = pu.VratiGodinuRazred(pomocni);
						String content[] = {cont[0], comboBox_1_1_1.getSelectedItem().toString()};
						poklapanje_godina = pu.PoklapanjeGodina(pomocni, datum, content);
					}catch(ClassNotFoundException | SQLException e2) {
						e2.printStackTrace();
					};
				
					if(comboBox_1_1_1.getSelectedItem().toString().equals("-") || poklapanje_godina) {
						pu.Greska("Pogresan unos.");
					}
					
					if((comboBox_1_1_1.getSelectedItem().toString().equals("-") || poklapanje_godina) == false) {
						
						try {
							pu.PostaviGodinuUpisa(comboBox_1_1_1.getSelectedItem().toString(), pomocni);
						} catch (ClassNotFoundException | SQLException e1) {
							e1.printStackTrace();
						}
				...
				if(textField_3.isEnabled()) {
					...
					if(textField_3.getText().equals("") == false) {
						try {
							pu.PostaviEmail(textField_3.getText(), pomocni);
						} catch (ClassNotFoundException | SQLException e1) {
							e1.printStackTrace();
						}
				...
				if(textField_4.isEnabled()) {
					...
					if(textField_4.getText().equals("") == false) {
						try {
							pu.PostaviSifru(textField_4.getText(), pomocni);
						} catch (ClassNotFoundException | SQLException e1) {
							e1.printStackTrace();
						}
				...
				if(comboBox_1_2.isEnabled()) {
					...
					boolean poklapanje_godina = false;
					try {
						datum = pu.VratiDatumRodjenja(pomocni);
						cont = pu.VratiGodinuRazred(pomocni);
						String content[] = {comboBox_1_2.getSelectedItem().toString(), cont[1]};
						poklapanje_godina = pu.PoklapanjeGodina(pomocni, datum, content);;
					} catch (ClassNotFoundException | SQLException e2) {
						e2.printStackTrace();
					}
					
					if(comboBox_1_2.getSelectedItem().toString().equals("-") || poklapanje_godina) {
						pu.Greska("Pogresan unos.");
					}
					if((comboBox_1_2.getSelectedItem().toString().equals("-") || poklapanje_godina) == false) {
						try {
							pu.PostaviRazred(comboBox_1_2.getSelectedItem().toString(), pomocni);
						} catch (ClassNotFoundException | SQLException e1) {
							e1.printStackTrace();
						}
