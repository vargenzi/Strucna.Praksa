...
public class BrisanjeOcene extends JFrame{
	...
	BrisanjeOcene(){
		...
		JButton btnSacuvaj = new JButton("OBRISI");
		...
		btnSacuvaj.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					provera = bo.ProveraPosotjanjaId(id, pomocni);
				...
				if(provera) {
					...
					try {
						String predmet = bo.TrazenjePodataka(pomocni, id);
						bo.Brisanje(id, predmet, pomocni);
					} catch (Exception e1) {
						e1.printStackTrace();
					}
